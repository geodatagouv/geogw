...
class Harvester extends EventEmitter {
  ...
  constructor(location, options = {}) {
    ...
    const { dublinCoreFallback } = options
    ...
    addHarvester(client.harvest({ schema: 'iso' }), 'inspire')

    if (dublinCoreFallback) {
      addHarvester(client.harvest({ schema: 'dc' }), 'dublin-core')
    }

    const recordIds = new Set()
    const ms = MultiStream(internalHarvesters, { objectMode: true })

    this.innerStream = ms.pipe(through.obj((record, enc, cb) => {
      const ignore = (reason) => {
