...
async function extractAndResolveLinks(metadata, currentLinks = []) {
  const rawLinksHref = metadata.links.map(rawLink => rawLink.href)
...
async function buildResourcesFromLinks(metadata, links) {
  ...
  metadata.links.forEach(rawLink => {
...
module.exports = { extractAndResolveLinks, buildResourcesFromLinks }
