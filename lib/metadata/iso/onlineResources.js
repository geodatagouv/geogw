...
function castOnlineResource({ name, href, protocol, description }) {
  try {
    const resource = new OnlineResource({ name, href, protocol, description })
    if (resource.isWfsFeatureType()) {
      return {
        service: resource.getNormalizedWfsServiceLocation(),
...
module.exports = { getAllOnLineResources, castOnlineResource }
