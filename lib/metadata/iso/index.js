...
function convert(originalRecord) {
  ...
  if (record.type !== 'service') {
    ...
    record.inspireTheme = getInspireThemeFromKeywords(originalRecord)
    ...
    // Links
    record.links = getAllOnLineResources(originalRecord).map(resource => ({
    ...
    if (isWFSService(originalRecord)) {
      ...
      if (record.serviceURL && coupledResources.length > 0) {
        record.featureTypes = coupledResources
          .map(coupledResource => ({
