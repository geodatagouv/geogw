...
function convertFromIso(record) {
    ...
    dataset.resourceId = get(record, 'identificationInfo.citation.identifier');
    ...
    // License
    dataset.license = getLicenseFromLinks(dataset.links)
      || getLicenseFromConstraints(get(record, 'identificationInfo.resourceConstraints', []))
      || getLicenseFromKeywords(dataset.keywords)
      || getLicenseFromCatalogs([]);
    ...
    // Covered territories
    dataset.coveredTerritories = getCoveredTerritories(record);
