...
function applyOrganizationsFilter(record) {
    ...
    const normalizedOrganizations = record.metadata.contributors.map(rawProducerName => {
      try {
        return normalizeProducerNames(rawProducerName);
      } catch (err) {
        return null;
      }
    });

    return record.set('organizations', uniq(compact(normalizedOrganizations)));
}
