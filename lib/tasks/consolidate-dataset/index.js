...
function getConsolidatedRecord(recordId) {
    return ConsolidatedRecord.findOne({ recordId }).exec()
        .then(record => {
            return record || new ConsolidatedRecord({ recordId, createdAt: new Date() });
        });
}
...
function applyResources(record, relatedResources) {
    ...
    relatedResources.forEach(function (resource) {
        ...
        } else if (resource.type === 'remote-resource' && resource.originHash !== record.recordHash) {
    ...
    return Promise.resolve(record
        .set('dataset.distributions', uniq(dist, 'uniqueId'))
