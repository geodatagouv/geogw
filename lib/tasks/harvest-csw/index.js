...
class CswHarvestJob extends ServiceSyncJob {
    ...
    _sync() {
        ...
        const location = this.service.location
          .replace('metadata.carmencarto.fr/geosource-', 'metadata.carmencarto.fr/geosource/');
        ...
        if (location.includes('geoportal/csw/discovery') || location.includes('tigeo') || location.includes('bdt.gers') || location.includes('geoplateforme17')) {
            harvesterOptions.forceConstraintLanguage = false;
        }
        if (location.includes('picto-occitanie')) harvesterOptions.omitElementSetName = true;
        if (location.includes('metadata.carmencarto.fr/geosource/11/')) harvesterOptions.omitElementSetName = true;
