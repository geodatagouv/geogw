...
function getCapabilities(service) {
  const client = wfs(service.location, {
      userAgent: 'WFSHarvester',
      timeout: 25
  });
  return client.capabilities()
    .then(capabilities => {
      debug('received capabilities');
      debug(JSON.stringify(capabilities, true, 2));
      return capabilities;
    })
    .catch(err => {
      debug('unable to retrieve capabilities');
...
class WfsLookupJob extends ServiceSyncJob {
  ...
  _sync() {
    ...
    debug('starting synchronization with WFS service');
